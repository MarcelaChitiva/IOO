...
public class Ranking{
    ...
    private ArrayList<Jugador> top10;
	public Ranking() {
		this.top10 = new ArrayList<Jugador>();
		try {
    ...
    public void clasificarJugador(Jugador jugador) {
        if (top10.size()<10 || top10.get(9).getPuntaje() < jugador.getPuntaje()) {
        	if (top10.size()>=10)
        		top10.remove(9);
        	top10.add(jugador);
        	Collections.sort(top10, new compararJugadores());
        	cambio = true;
        }
        if (cambio) {
        	cambio = false;
        	try {
        		...
        		for (int i=0; i<top10.size();i++) {
    	    		String line = top10.get(i).getNombre()+";"+String.valueOf(top10.get(i).getPuntaje());
    ...
    public ArrayList<Jugador> devolverTop10() {
    	return top10;
    }
